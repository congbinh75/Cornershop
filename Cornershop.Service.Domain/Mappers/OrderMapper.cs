...
public static class OrderMapper
{
    public static OrderDTO Map(this Order order)
    {
        return new OrderDTO
        {
            ...
            User = order.User.Map(),
            Code = order.Code,
            OrderDetails = order.OrderDetails.Select(OrderDetailMapper.Map).ToList(),
            TotalPrice = order.TotalPrice,
            Transactions = order.Transactions.Select(TransactionMapper.Map).ToList(),
    ...
    public static Order Map(this OrderDTO orderDTO)
    {
        return new Order
        {
            ...
            Code = orderDTO.Code,
            OrderDetails = orderDTO.OrderDetails.Select(OrderDetailMapper.Map).ToList(),
            TotalPrice = orderDTO.TotalPrice,
            Transactions = orderDTO.Transactions.Select(TransactionMapper.Map).ToList(),
